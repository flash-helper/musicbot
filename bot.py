...
# ==================== SHAZAM SERVICE (без библиотеки) ====================
class ShazamService:
    """Сервис распознавания музыки через Shazam API напрямую"""

    def __init__(self):
        self.api_url = "https://shazam.p.rapidapi.com/songs/detect"
        # Бесплатный способ - используем публичный API
        self.alt_url = "https://api.shazam.com/discovery/v5/en/US/android/-/tag"
    ...
    async def recognize_from_file(self, file_path: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Пробуем распознать через публичный API Shazam
            result = await self._recognize_shazam(audio_data)
            if result:
                return result
    ...
    async def _recognize_shazam(self, audio_data: bytes) -> Optional[Dict]:
        """Распознавание через Shazam API"""
        try:
            ...
            headers = {
                ...
                'User-Agent': 'Shazam/14.1.0 (iPhone; iOS 17.0; Scale/3.00)',
            }
            ...
            payload = {
                "signature": {
                    "uri": f"data:audio/ogg;base64,{audio_base64}",
                    "samplems": 3000
                }
            }

            async with aiohttp.ClientSession() as session:
                # Альтернативный метод через audd.io (бесплатный API)
                result = await self._recognize_audd(session, audio_data)
                if result:
                    return result
            ...
            logger.error(f"Shazam API error: {e}")
            return None

    async def _recognize_audd(self, session: aiohttp.ClientSession, audio_data: bytes) -> Optional[Dict]:
        """Распознавание через AudD API (бесплатный лимит)"""
        try:
            ...
            data.add_field('audio', audio_base64)
            data.add_field('return', 'apple_music,spotify')
            data.add_field('api_token', 'test')  # Бесплатный тестовый токен
...
class VKMusicService:
    """Сервис для работы с VK музыкой"""
...
def get_admin_menu() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="📢 Управление каналами", callback_data="admin_channels"))
    builder.row(InlineKeyboardButton(text="📨 Рассылка", callback_data="admin_broadcast"))
    builder.row(InlineKeyboardButton(text="⏰ Отложенные рассылки", callback_data="admin_scheduled"))
    builder.row(InlineKeyboardButton(text="👋 Приветствие", callback_data="admin_welcome"))
    return builder.as_markup()
...
def get_stats_kb() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="📈 Общая статистика", callback_data="stats_all"))
...
def get_channels_kb(channels: list) -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="🗑 Удалить канал", callback_data="delete_channel_menu"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_menu"))
    return builder.as_markup()
...
def get_broadcast_kb() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📝 Создать рассылку", callback_data="create_broadcast"))
    builder.row(InlineKeyboardButton(text="⏰ Отложить рассылку", callback_data="schedule_broadcast"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_menu"))
    return builder.as_markup()
...
def get_edit_broadcast_kb(broadcast_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📝 Изменить текст", callback_data=f"bedit_text_{broadcast_id}"))
    builder.row(InlineKeyboardButton(text="🖼 Изменить фото", callback_data=f"bedit_photo_{broadcast_id}"))
    builder.row(InlineKeyboardButton(text="🔘 Изменить кнопки", callback_data=f"bedit_buttons_{broadcast_id}"))
    builder.row(InlineKeyboardButton(text="⏰ Изменить время", callback_data=f"bedit_time_{broadcast_id}"))
    builder.row(InlineKeyboardButton(text="▶️ Отправить сейчас", callback_data=f"send_now_{broadcast_id}"))
    builder.row(
    ...
    try:
        ...
        if result:
            ...
            if result.get('shazam_url'):
                text += f"\n🔗 <a href=\"{result['shazam_url']}\">Открыть</a>"
...
async def view_vk_profile(callback: CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="🎵 Получить аудиозаписи", callback_data=f"get_vk_audio_{profile_id}"))
    builder.row(InlineKeyboardButton(text="🔗 Открыть в VK", url=profile.vk_url))
    builder.row(InlineKeyboardButton(text="🗑 Удалить", callback_data=f"delete_vk_{profile_id}"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_vk_profiles"))
...
async def edit_broadcast_photo_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🖼 <b>Отправьте фото или 'удалить':</b>", parse_mode="HTML")
...
async def edit_broadcast_buttons_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔘 <b>Введите кнопки:</b>\n\nТекст | URL\n\nИли 'удалить'", parse_mode="HTML")
...
async def edit_broadcast_time_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏰ <b>Новое время:</b>\n\nФормат: ДД.ММ.ГГГГ ЧЧ:ММ", parse_mode="HTML")
...
async def delete_broadcast(callback: CallbackQuery):
    ...
    if broadcasts:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_menu"))
        await callback.message.edit_text("⏰ Нет рассылок", reply_markup=builder.as_markup())
...
async def welcome_text_handler(message: Message, state: FSMContext):
    ...
    await message.answer("🖼 Отправьте фото или <b>пропустить</b>:", parse_mode="HTML")
